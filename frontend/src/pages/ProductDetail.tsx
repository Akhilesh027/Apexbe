...
const ProductDetail = () => {
    ...
    useEffect(() => {
        ...
        const fetchSimilarProducts = async (category, currentProductId) => {
            try {
                const res = await fetch(`https://website-backend-57f9.onrender.com/api/products?category=${category}&excludeId=${currentProductId}&limit=4`);
                const data = await res.json();
    ...
    const handleAddToCart = async () => {
        ...
        try {
            const res = await fetch("https://website-backend-57f9.onrender.com/api/cart/add", {
                method: "POST",
