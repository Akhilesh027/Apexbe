...
const Profile = () => {
  ...
  const fetchOrders = async () => {
    ...
    try {
      ...
      const response = await fetch(`https://website-backend-57f9.onrender.com/api/orders/user/${userData._id}`, {
        headers: {
  ...
  const fetchAddresses = async () => {
    ...
    try {
      const token = localStorage.getItem("token");
      const response = await fetch(`https://website-backend-57f9.onrender.com/api/user/address/${userData._id}`, {
        headers: {
          "Authorization": `Bearer ${token}`
        }
      });
                        ...
                        >
                          {order.orderItems?.[0]?.image && (
                            <div className="w-24 h-24 bg-muted rounded-md flex-shrink-0 overflow-hidden">
                              <img
                                src={`https://website-backend-57f9.onrender.com${order.orderItems[0].image}`}
