...
const MyOrders = () => {
  ...
  const fetchOrders = async () => {
    try {
      ...
      const response = await fetch(`https://website-backend-57f9.onrender.com/api/orders/user/${user._id}`, {
        headers: {
                    ...
                    {order.orderItems?.slice(0, 2).map((item, idx) => (
                      <div key={item._id || idx} className="flex gap-4">
                        <div className="w-20 h-20 bg-muted rounded-md flex-shrink-0 overflow-hidden">
                          <img 
                            src={item.image.startsWith('http') ? item.image : `https://website-backend-57f9.onrender.com${item.image}`} 
                            alt={item.name} 
