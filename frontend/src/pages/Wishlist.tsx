...
const Wishlist = () => {
  ...
  useEffect(() => {
    const fetchWishlist = async () => {
      ...
      try {
        const res = await axios.get(`https://website-backend-57f9.onrender.com/api/wishlist/${user._id}`);
        if (res.data.success) {
  ...
  const addToCart = async (item: any) => {
    try {
      ...
      const res = await axios.post("https://website-backend-57f9.onrender.com/api/cart/add", payload);
