...
const ProductCard = ({ product }: ProductCardProps) => {
    ...
    const fetchWishlistStatus = async () => {
      ...
      try {
        const res = await axios.get(
          `http://api.apexbee.in/api/wishlist/status?userId=${user._id}&productId=${product._id}`
        );
  ...
  const handleWishlist = async () => {
    ...
    try {
      const res = await axios.post("http://api.apexbee.in/api/wishlist/toggle", {
        userId: user._id,
  ...
  const handleAddToCart = async () => {
    ...
    try {
      ...
      const res = await axios.post("http://api.apexbee.in/api/cart/add", item);
