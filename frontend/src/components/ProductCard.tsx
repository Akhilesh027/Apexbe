...
const ProductCard = ({ product }: ProductCardProps) => {
    ...
    const fetchWishlistStatus = async () => {
      ...
      try {
        const res = await axios.get(
          `https://website-backend-57f9.onrender.com/api/wishlist/status?userId=${user._id}&productId=${product._id}`
        );
  ...
  const handleWishlist = async () => {
    ...
    try {
      const res = await axios.post("https://website-backend-57f9.onrender.com/api/wishlist/toggle", {
        userId: user._id,
  ...
  const handleAddToCart = async () => {
    ...
    try {
      ...
      const res = await axios.post("https://website-backend-57f9.onrender.com/api/cart/add", item);
